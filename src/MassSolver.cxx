...
The code is based on the following two publications:
 1. "Analytical solution of ttbar dilepton equations" published in Phys. Rev. D 73 054015 (2006)
 2. "Algebraic Approach to solve ttbar dilepton equations" Phys. Rev. D 72 095020 (2005)
...
bool MassSolver::solve( const TVector2& met, const Jet& bq1, const Jet& bq2,
                        Lepton& lep1, Lepton& lep2, double mt, 
                        std::vector<Neutrino>& nu1, std::vector<Neutrino>& nu2 ){
    /* lep1 = positive, lep2 = negative 

       mW1/2   = w mass (pdg value)
       mt1/mt2 = top_iter
    */
...
//    solve(ETmiss, b, bb, lp, lm, mW1, mW2, mt1 , mt2, 
//          &pnux, &pnuy, &pnuz, &pnubx, &pnuby, &pnubz, 
//          &cd_diff, cubic_single_root_cmplx);

    solve( met, bq1, bq2, lep1, lep2, mt, 
           &pnux, &pnuy, &pnuz, &pnubx, &pnuby, &pnubz,
           &cd_diff, cubic_single_root_cmplx);
    ...
    for( unsigned int i = 0 ; i < pnux.size() ; i++ ){
        Neutrino nu1_vec , nu2_vec;
        nu1_vec.p4.SetXYZM( pnux[i],  pnuy[i],  pnuz[i],  0 );
        nu2_vec.p4.SetXYZM( pnubx[i], pnuby[i], pnubz[i], 0 );

        TLorentzVector lvW1 = lep1.p4 + nu1_vec.p4;
        TLorentzVector lvW2 = lep2.p4 + nu2_vec.p4;
        TLorentzVector lvTop1 = lvW1 + bq1.p4;
        TLorentzVector lvTop2 = lvW2 + bq2.p4;

        if ((fabs(lvTop1.M() - m_topQuarkMass) < m_threshold) && (fabs(lvTop2.M() - m_topQuarkMass) < m_threshold) && 
            (fabs(lvW1.M()   - m_WMass) < m_threshold)        && (fabs(lvW2.M()   - m_WMass) < m_threshold)) {
...
void MassSolver::solve(const TVector2& met, const Jet& bq1, const Jet& bq2, 
...
//                       double* ETmiss, double* b, double* bb, double* lp, double* lm, 
//                       double m_WMass, double m_WMass, double mt, double mtb, 
//                       std::vector<double> *pnux, std::vector<double> *pnuy, std::vector<double> *pnuz, 
//                       std::vector<double> *pnubx, std::vector<double> *pnuby, std::vector<double> *pnubz, 
//                       std::vector<double> *cd_diff, int& cubic_single_root_cmplx) {
    ...
    double mtb = mt;   // mass of top quark in loop from AMWT (assumming m_top = m_antitop)
    double ETmiss[2] = { met.Px(), met.Py() };
    double b[4]  = { bq1.p4.E() , bq1.p4.Px() , bq1.p4.Py() , bq1.p4.Pz() };
    double bb[4] = { bq2.p4.E() , bq2.p4.Px() , bq2.p4.Py() , bq2.p4.Pz() };
    double lp[4] = { lep1.p4.E() , lep1.p4.Px() , lep1.p4.Py() , lep1.p4.Pz() };
    double lm[4] = { lep2.p4.E() , lep2.p4.Px() , lep2.p4.Py() , lep2.p4.Pz() };
    ...
    // double mbnu = sqrt(sqr(mt)-sqr(m_WMass)+sqr(mb)+sqr(mlp)-sqr(mblp));


    double a1 = (b[0]+lp[0])*(sqr(m_WMass)-sqr(mlp))-lp[0]*(sqr(mt)-sqr(mb)-sqr(mlp))+2.*b[0]*sqr(lp[0])
  ...
  //c00*pnuy^2+(c10*pnux+c11)*pnuy+(c20*pnux^2+c21*pnux+c22)
    double c22 = sqr((sqr(m_WMass)-sqr(mlp))*a4)-4.*(sqr(lp[0])-sqr(lp[3]))*sqr(a1)
                 -4.*(sqr(m_WMass)-sqr(mlp))*lp[3]*a1*a4; //c1
    double c21 = -8.*(sqr(lp[0])-sqr(lp[3]))*a1*a2+4.*(sqr(m_WMass)-sqr(mlp))*(lp[1]*sqr(a4)-lp[3]*a2*a4)
                 -8.*lp[1]*lp[3]*a1*a4; //c2
    double c11 = -8.*(sqr(lp[0])-sqr(lp[3]))*a1*a3+4.*(sqr(m_WMass)-sqr(mlp))*(lp[2]*sqr(a4)-lp[3]*a3*a4)
                 -8.*lp[2]*lp[3]*a1*a4; //c3
    double c20 = -4.*(sqr(lp[0])-sqr(lp[1]))*sqr(a4)-4.*(sqr(lp[0])-sqr(lp[3]))*sqr(a2)
                 -8.*lp[1]*lp[3]*a2*a4; //c4
    double c10 = -8.*(sqr(lp[0])-sqr(lp[3]))*a2*a3+8.*lp[1]*lp[2]*sqr(a4)-8.*lp[1]*lp[3]*a3*a4
                 -8.*lp[2]*lp[3]*a2*a4; //c5
    double c00 = -4.*(sqr(lp[0])-sqr(lp[2]))*sqr(a4)-4.*(sqr(lp[0])-sqr(lp[3]))*sqr(a3)
                 -8.*lp[2]*lp[3]*a3*a4; //c6


    double b1 = (bb[0]+lm[0])*(sqr(m_WMass)-sqr(mlm))-lm[0]*(sqr(mtb)-sqr(mbb)-sqr(mlm))
                +2.*bb[0]*sqr(lm[0])-2.*lm[0]*(bb[1]*lm[1]+bb[2]*lm[2]+bb[3]*lm[3]);
    double b2 = 2.*(bb[0]*lm[1]-lm[0]*bb[1]);
    double b3 = 2.*(bb[0]*lm[2]-lm[0]*bb[2]);
    double b4 = 2.*(bb[0]*lm[3]-lm[0]*bb[3]);


    double dp22 = sqr((sqr(m_WMass)-sqr(mlm))*b4)-4.*(sqr(lm[0])-sqr(lm[3]))*sqr(b1)
                  -4.*(sqr(m_WMass)-sqr(mlm))*lm[3]*b1*b4; //d1
    double dp21 = -8.*(sqr(lm[0])-sqr(lm[3]))*b1*b2+4*(sqr(m_WMass)-sqr(mlm))*(lm[1]*sqr(b4)-lm[3]*b2*b4)
                  -8.*lm[1]*lm[3]*b1*b4; //d2
    double dp11 = -8.*(sqr(lm[0])-sqr(lm[3]))*b1*b3+4*(sqr(m_WMass)-sqr(mlm))*(lm[2]*sqr(b4)-lm[3]*b3*b4)
                  -8.*lm[2]*lm[3]*b1*b4; //d3
    double dp20 = -4.*(sqr(lm[0])-sqr(lm[1]))*sqr(b4)-4.*(sqr(lm[0])-sqr(lm[3]))*sqr(b2)
                  -8.*lm[1]*lm[3]*b2*b4; //d4
    double dp10 = -8.*(sqr(lm[0])-sqr(lm[3]))*b2*b3+8.*lm[1]*lm[2]*sqr(b4)-8.*lm[1]*lm[3]*b3*b4
                  -8.*lm[2]*lm[3]*b2*b4; //d5
    double dp00 = -4.*(sqr(lm[0])-sqr(lm[2]))*sqr(b4)-4.*(sqr(lm[0])-sqr(lm[3]))*sqr(b3)
                  -8.*lm[2]*lm[3]*b3*b4; //d6
  ...
  // expressions c0,1,2,d0,1,2 are polynomials in pnux of degree 2
  // to be multiplied out below

 
    std::vector<double> polx(5);
    ...
    for (unsigned int i=0; i<pnuxt.size(); ++i) {
        ...
        if (fabs(lpbz_diff)<epsilon) { // circumvent singularity
            error = algebraic_pz(b, lp, mt, mb, mlp, thispnux, thispnuy, &thispnuz); 
        ...
        if (fabs(lmbbz_diff)<epsilon) { // circumvent singularity
            error = algebraic_pz(bb, lm, mtb, mbb, mlm, thispnubx, thispnuby, &thispnubz); 
            if (error == -1) continue;  // next pnux, pnubx solutions
        }
        else {
...
//////////////////////////////////////////////////////////////////////////////////////////
void MassSolver::quartic(std::vector<double> const & polx, std::vector<double> *pnux, int& cubic_single_root_cmplx) {
    /* Quartic function */
    std::vector<double> polxt(polx.size());
    cubic_single_root_cmplx = 0;
    if (polx[4] == 0.)
        cubic(polx, pnux);
    else {
        ...
        if (polxt[0] == 0.) {
            ...
            if (g == 0.) {
                ...
                std::vector<double> polxt2(3);
                polxt2[0]=g;
                polxt2[1]=e;
                polxt2[2]=1.;
                std::vector<double> polxt3;
                ...
                if (polxt3.size()==1 && polxt3[0]<0.) {
                    cma::WARNING("Quartic: single cubic h^2 solution is negative! (unique sol="+
                                 std::to_string(polxt3[0])+") => complex root h");
                ...
                std::vector<double> polxt4(3);
                polxt4[0]=j;
                polxt4[1]=h;
                polxt4[2]=1.;
                std::vector<double> polxt5;
...
//////////////////////////////////////////////////////////////////////////////////////////
void MassSolver::cubic(std::vector<double> const & polx, std::vector<double> *pnux) {
    /* Cubic function */
    if (polx[3] == 0.)
        quadratic(polx, pnux);
    else {
        ...
        if (q == 0.) { //single root of degree three
            ...
            (*pnux).push_back(-2.*sqrt(q)*cos((theta+2.*m_Pi)/3.)-polx[2]/3.);
            (*pnux).push_back(-2.*sqrt(q)*cos((theta+4.*m_Pi)/3.)-polx[2]/3.);
        }
        else { // 1 real root
...
//////////////////////////////////////////////////////////////////////////////////////////
void MassSolver::quadratic(std::vector<double> const & polx, std::vector<double> *pnux) {
    /* Quadratic function */
...
///////////////////////////////////////////////////////////////////////////////////////////
// Ansatz of Algebraic Approach to solve ttbar dilepton equations: 
//   Phys. Rev. D 72, 095020 (2005)
int MassSolver::algebraic_pz(double* b, double* lp, double mt, double mb, double mlp, 
  ...
  // a1=a11+a12*nu[1]+a13*nu[2];
    std::vector<double> a1(3);
    a1[0]=(sqr(m_WMass)-sqr(mlp))*lp[3]*0.5 / (sqr(lp[0])-sqr(lp[3]));
  ...
  //a2=a21+a22*nu[1]+a23*nu[2]+a24*nu[1]^2+a25**nu[1]*nu[2]+a26*nu[2]^2
    std::vector<double> a2(6);
    a2[0]=(quad(m_WMass)+quad(mlp)-2.*sqr(m_WMass)*sqr(mlp))*0.25 / (sqr(lp[0])-sqr(lp[3]));
    a2[3]=-(sqr(lp[0])-sqr(lp[1])) / (sqr(lp[0])-sqr(lp[3]));
    a2[5]=-(sqr(lp[0])-sqr(lp[2])) / (sqr(lp[0])-sqr(lp[3]));
    a2[1]=(sqr(m_WMass)-sqr(mlp))*lp[1] / (sqr(lp[0])-sqr(lp[3]));
    a2[2]=(sqr(m_WMass)-sqr(mlp))*lp[2] / (sqr(lp[0])-sqr(lp[3]));
  ...
  //determine first temporary pnuz, pnubz
    ...
    std::vector<double> pnuz_a, pnuz_b;
    ...
    if (radicant>=0.) {
        ...
        cma::ERROR("MassSolver: algebraic_pz: pnuz_a radicant = "
                   +std::to_string(radicant)+" should never happen!");
    }
    ...
    if (radicant>=0.) {
        ...
        cma::ERROR("MassSolver: algebraic_pz: pnuz_b radicant = "
                   +std::to_string(radicant)+" should never happen!");
    }
...
//////////////////////////////////////////////////////////////////////////////////////////
double MassSolver::evalterm1(std::vector<double> *a1, double pnux, double pnuy) {
    return (*a1)[0]+(*a1)[1]*pnux+(*a1)[2]*pnuy;
}

//////////////////////////////////////////////////////////////////////////////////////////
double MassSolver::evalterm2(std::vector<double> *a2, double pnux, double pnuy) {
